...
/**
 * Minimal shape `WastePolicy` needs from a candidate resource: only `tags`,
 * for the exclusion-tag/tag-value checks in `evaluate()`. Deliberately not
 * `WastedResource` itself (which also carries `kind`, `region`, `costEstimate`,
 * etc.) — that type enumerates cloudrift core's own 44-scanner catalog and
 * lives in `cloud-cost-domain`, which depends on this package for `AwsRegion`/
 * `CostEstimate`/`PricingPort`. Constraining on the full type would create a
 * circular package dependency for a single field this class actually reads.
 */
 ...
 */
export abstract class WastePolicy<T extends TaggedResource> {
