...
describe('EcrRepositoryEmpty', () => {
  ...
  it('exposes the remaining props', () => {
    ...
    const repo = makeRepo({ region, accountId: '999999999999', createdAt, detectedAt, tags: { env: 'prod' } });
    expect(repo.region).toBe(region);
    expect(repo.accountId).toBe('999999999999');
    expect(repo.createdAt).toBe(createdAt);
    expect(repo.detectedAt).toBe(detectedAt);
    expect(repo.tags).toEqual({ env: 'prod' });
  });
});
