...
function allKindFindings(): WastedResource[] {
  return [
    new EbsVolume({ volumeId: 'vol-1', region, accountId, sizeGb: 100, volumeType: 'gp3', state: 'available', createTime: OLD, detectedAt: now, tags, monthlyCostUsd: 8, wasteReason: 'unattached' }),
    new EbsVolume({ volumeId: 'vol-2', region, accountId, sizeGb: 500, volumeType: 'io1', state: 'available', createTime: OLD, detectedAt: now, tags, monthlyCostUsd: 40, wasteReason: 'unattached' }),
    new ElasticIp({ allocationId: 'eipalloc-1', publicIp: '1.2.3.4', region, accountId, detectedAt: now, tags, monthlyCostUsd: 3.6, wasteReason: 'unassociated' }),
    new RdsInstance({ dbInstanceIdentifier: 'db-1', region, accountId, dbInstanceClass: 'db.t3.micro', engine: 'postgres', dbInstanceStatus: 'stopped', allocatedStorageGb: 20, storageType: 'gp3', multiAZ: false, detectedAt: now, tags, monthlyCostUsd: 15, wasteReason: 'stopped (storage and backups still billed)' }),
    new LoadBalancer({ arn: 'arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/lb-1/abc', name: 'lb-1', region, accountId, type: 'application', createdTime: OLD, detectedAt: now, registeredTargetCount: 0, tags, monthlyCostUsd: 16, wasteReason: 'no registered targets' }),
    new Ec2Instance({ instanceId: 'i-1', region, accountId, instanceType: 't3.micro', state: 'stopped', launchTime: OLD, detectedAt: now, stoppedSince: OLD, attachedVolumes: [{ volumeId: 'vol-3', sizeGb: 8, volumeType: 'gp3' }], tags, monthlyCostUsd: 0.64, wasteReason: 'stopped (attached EBS still billed)' }),
    new EbsSnapshot({ snapshotId: 'snap-1', region, accountId, sourceVolumeId: 'vol-gone', sourceVolumeExists: false, sizeGb: 100, startTime: OLD, detectedAt: now, description: 'old snapshot', tags, monthlyCostUsd: 5, wasteReason: 'source volume deleted' }),
    new EbsSnapshot({ snapshotId: 'snap-2', region, accountId, sourceVolumeId: 'vol-gone-2', sourceVolumeExists: false, sizeGb: 50, startTime: OLD, detectedAt: now, description: 'another old snapshot', tags, monthlyCostUsd: 2.5, wasteReason: 'source volume deleted' }),
    ...
    new LogGroup({ logGroupName: '/aws/lambda/my-fn', region, accountId, storedBytes: 1024 ** 3, creationTime: OLD, detectedAt: now, tags, monthlyCostUsd: 0.03, wasteReason: 'no retention policy' }),
    new OrphanedEni({ networkInterfaceId: 'eni-1', region, accountId, vpcId: 'vpc-1', subnetId: 'subnet-1', status: 'available', detectedAt: now, tags, wasteReason: 'not attached' }),
    new S3Bucket({ bucketName: 'my-bucket', region, accountId, sizeBytes: 1024 ** 4, hasLifecyclePolicy: false, creationDate: OLD, detectedAt: now, tags, monthlyCostUsd: 1.5, wasteReason: 'no lifecycle policy configured' }),
