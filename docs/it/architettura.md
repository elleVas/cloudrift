...
- **`dead-resources.composition.ts`** rispecchia la forma di `analyze-waste.composition.ts` a una frazione della dimensione: `buildScanners()` è un semplice array di 18 entry (non spezzato in file come [ADR-0077](../adr/0077-scanner-registry-split-on-pricing-seam.md) ha fatto per il registry cost-waste da 44 entry — 18 non lo giustifica ancora), e il filtro `scannerKinds` (da `--scanners` o dal multiselect del wizard) funziona come `AnalysisContext.scannerKinds`.
...
- **Il protocollo MCP vive in `apps/cli`** (`scope:app`), l'unico progetto autorizzato a vedere tutti i layer — stesso motivo per cui il parsing delle opzioni di Commander vive lì e non in un package application. `mcp.composition.ts` costruisce i quattro use case concreti, già collegati ad AWS (riusando le composition root esistenti tali e quali); `mcp.command.ts` è puro collante di protocollo: sette tool (`analyze_cloudrift`, `analyze_cloud_waste`, `analyze_dead_resources`, `analyze_resource_security`, `get_cost_trend`, `get_resource_types`, `get_required_iam_permissions`) registrati su un `McpServer` con schema Zod. `get_cost_trend` e i tre tool per-dominio `analyze_*` non passano da `AggregateAnalysisUseCase` — ciascuno è una query su un solo dominio, quindi collega direttamente la composition root di quel dominio (`CostTrendUseCase`/`defaultCostAnalyticsDeps` per il primo, `defaultAnalyzeDeps`/`defaultDeadResourcesDeps`/`defaultResourceSecurityDeps` per gli altri tre), le stesse usate dai comandi `trend`/`analyze`/`dead-resources`/`resource-security`, meno il prompt interattivo `confirmCostExplorerCharge` di `trend` (non c'è un terminale con cui confermare via MCP). `resolveMcpScope()` in `mcp.composition.ts` estrae il preambolo di caricamento config/parsing region/risoluzione account condiviso da tutti e quattro i tool di analisi, una volta che una quarta copia quasi identica avrebbe altrimenti fatto la sua comparsa.
...
- **Ogni tool registrato è avvolto in un `withErrorBoundary` condiviso**, che intercetta un *throw non gestito* (a differenza del percorso *atteso* `Result.fail` sopra) e lo converte nella stessa forma `isError` di `CallToolResult`. Applicato uniformemente a tutti e sette i tool, non solo ai cinque che fanno chiamate AWS — i due tool statici non possono lanciare eccezioni oggi, ma il boundary li copre comunque così una futura modifica a uno di essi non richiede di ricordarsi di aggiungerlo. Vedi [ADR-0086](../adr/0086-mcp-tool-handlers-error-boundary.md).
...
## Trend store locale: `history`

Un nuovo package `scope:shared`, `shared-trend-store` (`libs/shared/trend-store`), non un quinto bounded context e non parte di alcun dominio — conosce solo `{ domain, generatedAt, payload }`, nessun tipo DTO specifico di dominio, stesso ragionamento su cui è costruito `shared-scan-coordination` (ADR-0095).

```
analyzeWasteCommand / deadResourcesCommand / resourceSecurityCommand   (apps/cli)
    │ dopo aver prodotto il report (lo stesso JSON già usato per --format json / --json)
    ▼
persistTrendSnapshot(accountId, { domain, generatedAt, payload })       (shared-trend-store)
    │ best-effort: try/catch, un fallimento di scrittura non fa mai fallire il comando
    ▼
~/.cloudrift/trends/<account-id>.db                                    (node:sqlite, caricato lazy)
...
- **La chiamata di persistenza vive in ogni comando CLI, non dentro `ScanCoordinatorUseCase`.** Il coordinatore è `scope:shared` e conosce solo `ScannableRegion`/`TFinding`/`TSummary` — non ha alcun concetto di `accountId` (risolto separatamente, via STS, in ogni comando). Il livello comando è il primo punto che ha sia il report finito sia l'ID account risolto.
- **Il payload salvato è esattamente l'output `--format json` di ciascun dominio** (`formatWasteReportAsJson`/`formatDeadResourcesReportAsJson`/`formatResourceSecurityReportAsJson`), riusato invece di ri-derivato — il DTO è già l'unica serializzazione JSON-safe corretta di un summary (vedi la sezione `cloud-cost/application` sopra).
- **`node:sqlite` è caricato tramite un `await import('node:sqlite')` lazy, mai un import statico.** Richiede Node ≥22.5, ma l'`engines.node` pubblicato di `apps/cli` è `>=20`; un import statico di un modulo core inesistente lancia un errore al momento del caricamento, che romperebbe l'intera CLI (ogni comando) per chi è ancora su Node 20/21. Il caricamento lazy fa sì che il trend store semplicemente non persista mai nulla su Node vecchio, senza impattare nient'altro.
- **Un file SQLite per account AWS**, non un unico file condiviso con una colonna `account_id` — coerente col fatto che le scansioni sono già account-scoped, ed evita che la scrittura di un account contenda mai con quella di un altro.

Vedi [ADR-0099](../adr/0099-local-trend-store.md) (in inglese) per la decisione completa (incluti i compromessi su granularità/retention) e [utilizzo.md](utilizzo.md#history--storico-locale-delle-scansioni) per i flag del comando `history`.
