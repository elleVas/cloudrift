...
- **`dead-resources.composition.ts`** rispecchia la forma di `analyze-waste.composition.ts` a una frazione della dimensione: `buildScanners()` è un semplice array di 13 entry (non spezzato in file come [ADR-0077](../adr/0077-scanner-registry-split-on-pricing-seam.md) ha fatto per il registry cost-waste da 43 entry — 13 non lo giustifica ancora), e il filtro `scannerKinds` (da `--scanners` o dal multiselect del wizard) funziona come `AnalysisContext.scannerKinds`.
