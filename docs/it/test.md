...
```
        ...
        ├─────────────────────────┤
        │  Infra (contract test)  │   fixture di risposte reali riprodotte: shape → findings, 44/44
        ├─────────────────────────┤
        ...
        └─────────────────────────┘
        ┌─────────────────────────┐
        │  e2e LocalStack (free)  │   scripts/e2e-localstack.mjs (questo doc), 17/44 scanner
        ├─────────────────────────┤
        │  Verifica manuale AWS   │   scripts/verify-against-aws.mjs (questo doc)
        ├─────────────────────────┤
        │  Verifica AWS reale     │   harness CDK esterno (vedi sotto), 36/44 scanner
        └─────────────────────────┘
```

### Gate di coverage

Tutti i 16 `jest.config.cjs` impostano un `coverageThreshold` reale — **80%** (statements/branches/functions/lines) per i progetti domain e application, **60%** per i progetti infrastructure e per `apps/cli` — calcolato su ogni file `.ts` sotto `src/`, non solo sui file che uno spec importa (`collectCoverageFrom` esclude esplicitamente solo `*.spec.ts` e il barrel `index.ts`, puro re-export senza logica propria). `pnpm nx run <project>:test` fallisce se un progetto scende sotto la propria soglia. Vedi [ADR-0090](../adr/0090-enforced-coverage-thresholds.md) per il perché di questo calcolo (un `collectCoverageFrom` più stretto nascondeva silenziosamente interi file non testati dalla percentuale) e per il perché i numeri sono stati raggiunti scrivendo test reali, non calibrando la soglia sulla copertura già esistente.
...
### Trend store locale — `history` (2026-07-30, vedi [ADR-0099](../adr/0099-local-trend-store.md), in inglese)

- **`shared-trend-store`**: [`trend-store.spec.ts`](../../libs/shared/trend-store/src/trend-store.spec.ts) copre il round-trip persist/read, l'isolamento del file per-account, l'ordinamento dal più recente con i filtri `--domain`/`--limit`, la policy full-snapshot-forever (niente viene mai sovrascritto), e il contratto di scrittura best-effort (una directory non creabile non lancia mai un errore). La soglia di coverage è 60%, non l'80% consueto di questo repo, documentato inline nel `jest.config.cjs` del package: l'unico ramo non testato è un default rivolto al sistema operativo (`os.homedir()`) che l'ambiente di test node di Jest non permette di sovrascrivere in modo affidabile — riassegnare `process.env.HOME` a metà test non si propaga a quel binding nativo sotto Jest, confermato empiricamente (anche `jest.spyOn` fallisce apertamente: "Cannot redefine property: homedir").
- **CLI e2e**: [`history.command.spec.ts`](../../apps/cli/src/commands/history.command.spec.ts) pilota il comando con `resolveAccountId`/`readSnapshots` finti (nessun AWS, nessun file SQLite reale), coprendo la validazione di `--format`/`--domain`/`--limit`, l'avviso di fallita risoluzione account via STS, e il rendering sia table che json incluso il messaggio a storico vuoto. [`analyze-waste.command.spec.ts`](../../apps/cli/src/commands/analyze-waste.command.spec.ts), [`dead-resources.command.spec.ts`](../../apps/cli/src/commands/dead-resources.command.spec.ts) e [`resource-security.command.spec.ts`](../../apps/cli/src/commands/resource-security.command.spec.ts) hanno ciascuno un nuovo test che verifica che `persistTrendSnapshot` venga chiamato con `accountId`/`domain`/`payload` corretti dopo una scansione — `shared-trend-store` è mockato (`jest.mock`) in tutti e tre gli spec, dato che l'implementazione reale scrive su `~/.cloudrift/trends/<account-id>.db` e un test unitario non deve mai toccare la home directory reale dello sviluppatore.
- **Non ancora fatto — verifica su AWS reale.** A differenza di ogni dominio sopra, questa feature non è ancora stata eseguita contro un account AWS reale; il piano è verificarla lì prima della prossima release, non rilasciarla solo sulla fiducia dei test unitari.

### Riepilogo trend in CI (2026-08-21, vedi [ADR-0104](../adr/0104-trend-summary-actions-cache.md), in inglese)

- **Formatter**: [`trend-history.markdown-formatter.spec.ts`](../../apps/cli/src/formatters/trend-history.markdown-formatter.spec.ts) copre l'helper `sparkline()` direttamente — nessun valore, un singolo valore, una serie tutta uguale (deve renderizzare il livello centrale piatto invece di una divisione per zero), e una serie strettamente crescente che copre l'intero intervallo basso-alto — oltre alla didascalia con il conteggio dei run di `formatTrendHistoryAsMarkdown` (singolare "run" vs plurale), le righe della tabella Markdown, e il formato della didascalia in dollari vs conteggio semplice per i domini senza `monthlyWasteUsd` (dead-resources/resource-security).
- **CLI e2e**: [`history.command.spec.ts`](../../apps/cli/src/commands/history.command.spec.ts) copre l'output dell'elenco semplice con `--format markdown` e il rifiuto esplicito di `--format markdown` combinato con `--compare` (non esiste un renderer Markdown per il confronto a due run).
- **Non coperto da alcun test automatico — l'input `trend-summary` di `action.yml`.** Non esiste un harness che esegua la composite action in sé (l'harness e2e su LocalStack pilota solo il binario CLI compilato, mai un vero runner GitHub Actions), quindi il collegamento `actions/cache/restore`+`actions/cache/save`, l'idioma chiave-univoca-più-prefisso e il comportamento di scoping cache per branch/fork sono verificati solo contro la semantica documentata di `actions/cache`, non eseguiti dal vivo. Verifica manualmente contro un run reale del workflow prima di farci affidamento per un annuncio di release.

### Notifiche — Slack / webhook / email / commento PR GitHub ([ADR-0103](../adr/0103-github-pr-comment-notifier.md), in inglese)

- **`shared-notifications`**: uno spec per canale — [`slack-webhook.notifier.spec.ts`](../../libs/shared/notifications/src/slack-webhook.notifier.spec.ts), [`generic-webhook.notifier.spec.ts`](../../libs/shared/notifications/src/generic-webhook.notifier.spec.ts), [`email.notifier.spec.ts`](../../libs/shared/notifications/src/email.notifier.spec.ts), [`github-comment.notifier.spec.ts`](../../libs/shared/notifications/src/github-comment.notifier.spec.ts) — ciascuno con `fetch`/`nodemailer` mockati, che copre il percorso felice, una risposta non-2xx, e un errore di rete che risolve sempre in `Result.fail` invece di lanciare un'eccezione (un canale rotto non deve mai far crashare lo scan che sta segnalando). [`should-notify.spec.ts`](../../libs/shared/notifications/src/should-notify.spec.ts) copre i tre gate indipendenti "vale la pena segnalarlo" (`shouldNotifyOnSeverity`, `hasRegressed`, `shouldNotifyOnCost`) che ogni comando riusa per decidere se far scattare un canale qualsiasi.
- **CLI e2e**: [`notifications.spec.ts`](../../apps/cli/src/commands/notifications.spec.ts) pilota `dispatchNotifications` con `shared-notifications` completamente mockato, coprendo: ogni canale che scatta quando il suo flag e le sue env var sono presenti, il percorso di avviso-e-skip quando mancano le env var richieste (`SLACK_WEBHOOK_URL`, `CLOUDRIFT_WEBHOOK_URL`, le quattro `CLOUDRIFT_SMTP_*`, `GITHUB_TOKEN`/`GITHUB_REPOSITORY`/`GITHUB_EVENT_PATH`), un invio fallito che viene loggato invece di lanciato, il canale GitHub che salta anche quando l'evento scatenante non ha un `pull_request` (un run push/cron), e più canali che scattano insieme in un'unica chiamata. `topFindingLines` (helper condiviso "top N finding, più severi per primi" che alimenta i corpi di webhook/email/commento GitHub) ha il suo blocco describe nello stesso file.
- **Verifica nel mondo reale**: Slack è stato testato dal vivo contro un webhook reale (trovati e corretti tre problemi — il parser `mrkdwn` di Slack che mangia una coppia `*`/`_` dentro una stringa grezza di una policy IAM, il design precedente con tutte le severità che era fonte di alert fatigue, e la formattazione colore/markdown). Email/SMTP e il canale commento PR GitHub sono verificati solo a livello di test unitari; un'esecuzione nel mondo reale di ciascuno resta aperta.
...
Lo scope è 17 dei 44 scanner (vedi [ADR-0002](../adr/0002-localstack-e2e-scope.md), [ADR-0036](../adr/0036-ec2-underutilized-excluded-from-localstack-e2e.md) e [ADR-0040](../adr/0040-localstack-bumped-4-14-0-cloudwatch-fixed.md)):
...
**Copertura attuale: 36 dei 44 scanner hanno trovato uno spreco reale su un account AWS live** (33 originali + `ami-unused`, `ecr-image-untagged`, `s3-multipart-upload-abandoned`, confermati il 2026-07-22 tramite l'harness `cloudrift-cdk-test`). I restanti 8 si dividono in tre tipi di gap diversi:
...
- `codepipeline-pipeline-stale` (aggiunto il 2026-07-23, un costo fisso a riposo di $1/mese per pipeline — spostato qui dalla lista candidati dead-resources invece di trattarlo come flag di hygiene a $0) non ha ancora avuto nessun giro su AWS reale; unit test e contract test a fixture replay sono verdi, la verifica live è in attesa del prossimo ciclo `cloudrift-cdk-test`.
