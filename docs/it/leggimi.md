...
> **Nota onesta (verifica su AWS reale):** 33 dei 38 scanner sono stati verificati contro un account AWS reale; 5 restano non verificati per scelta, non per dimenticanza — `rds-underutilized`, `environment-ghost`, `sqs-dlq-abandoned`, `aurora-serverless-overprovisioned` ed `eks-node-overprovisioned` richiedono tutti risorse che hanno accumulato pattern d'uso reali e organici per 7-14 giorni, cosa che uno stack di test sintetico di breve durata non può produrre. Tutti e 5 restano comunque coperti da unit test e contract test a fixture replay (risposte AWS mockate) — il gap è specificamente "confermato contro un account AWS live", non "non testato". Vedi [test.md](test.md#stato-della-verifica-su-aws-reale-più-ampia-di-verify-against-awsmjs) per il dettaglio completo.
