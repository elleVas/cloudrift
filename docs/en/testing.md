...
### Local trend store — `history` (2026-07-30, see [ADR-0099](../adr/0099-local-trend-store.md))

- **`shared-trend-store`**: [`trend-store.spec.ts`](../../libs/shared/trend-store/src/trend-store.spec.ts) covers round-trip persist/read, per-account file isolation, most-recent-first ordering with `--domain`/`--limit` filters, the full-snapshot-forever policy (nothing is ever overwritten), and the best-effort write contract (a directory that can't be created never throws). Coverage threshold is 60%, not this repo's usual 80%, documented inline in the package's `jest.config.cjs`: its only untested branch is an OS-facing default (`os.homedir()`) that Jest's node test environment can't reliably let a test override — `process.env.HOME` reassignment mid-test does not propagate to that native binding under Jest, confirmed empirically (`jest.spyOn` also fails outright: "Cannot redefine property: homedir").
- **CLI e2e**: [`history.command.spec.ts`](../../apps/cli/src/commands/history.command.spec.ts) drives the command with fake `resolveAccountId`/`readSnapshots` deps (no AWS, no real SQLite file), covering `--format`/`--domain`/`--limit` validation, the STS-account-resolution-failed warning, and both table/json rendering including the empty-history message. [`analyze-waste.command.spec.ts`](../../apps/cli/src/commands/analyze-waste.command.spec.ts), [`dead-resources.command.spec.ts`](../../apps/cli/src/commands/dead-resources.command.spec.ts), and [`resource-security.command.spec.ts`](../../apps/cli/src/commands/resource-security.command.spec.ts) each got a new test asserting `persistTrendSnapshot` is called with the right `accountId`/`domain`/`payload` after a scan — `shared-trend-store` is `jest.mock`ed in all three specs, since the real implementation writes to `~/.cloudrift/trends/<account-id>.db` and a unit test must never touch the developer's actual home directory.
- **Not yet done — real-AWS verification.** Unlike every domain above, this feature hasn't been run against a real AWS account yet; the plan is to verify it there before the next release, not to ship on unit-test confidence alone.

### Trend summary in CI (2026-08-21, see [ADR-0104](../adr/0104-trend-summary-actions-cache.md))

- **Formatter**: [`trend-history.markdown-formatter.spec.ts`](../../apps/cli/src/formatters/trend-history.markdown-formatter.spec.ts) covers the `sparkline()` helper directly — no values, a single value, an all-equal series (each must render the flat middle level rather than a division-by-zero), and a strictly increasing series spanning the full low-to-high range — plus `formatTrendHistoryAsMarkdown`'s run-count caption (singular "run" vs. plural), the Markdown table rows, and the dollar-vs-plain-count caption format for domains with no `monthlyWasteUsd` (dead-resources/resource-security).
- **CLI e2e**: [`history.command.spec.ts`](../../apps/cli/src/commands/history.command.spec.ts) covers `--format markdown`'s plain-listing output and the explicit rejection of `--format markdown` combined with `--compare` (no Markdown renderer exists for the two-run diff).
- **Not covered by any automated test — `action.yml`'s `trend-summary` input.** There is no harness that runs the composite action itself (the LocalStack e2e harness only drives the built CLI binary, never an actual GitHub Actions runner), so the `actions/cache/restore`+`actions/cache/save` wiring, the unique-key-plus-prefix-match idiom, and the branch/fork cache-scoping behavior are verified against GitHub's documented `actions/cache` semantics only, not exercised live. Verify manually against a real workflow run before relying on it for a release announcement.

### Notifications — Slack / webhook / email / GitHub PR comment ([ADR-0103](../adr/0103-github-pr-comment-notifier.md))

- **`shared-notifications`**: one spec per channel — [`slack-webhook.notifier.spec.ts`](../../libs/shared/notifications/src/slack-webhook.notifier.spec.ts), [`generic-webhook.notifier.spec.ts`](../../libs/shared/notifications/src/generic-webhook.notifier.spec.ts), [`email.notifier.spec.ts`](../../libs/shared/notifications/src/email.notifier.spec.ts), [`github-comment.notifier.spec.ts`](../../libs/shared/notifications/src/github-comment.notifier.spec.ts) — each with `fetch`/`nodemailer` mocked, covering the happy path, a non-2xx response, and a network error always resolving to `Result.fail` rather than throwing (a broken channel must never crash the scan it's reporting on). [`should-notify.spec.ts`](../../libs/shared/notifications/src/should-notify.spec.ts) covers the three independent "is this worth reporting" gates (`shouldNotifyOnSeverity`, `hasRegressed`, `shouldNotifyOnCost`) that every command reuses to decide whether to fire any channel at all.
- **CLI e2e**: [`notifications.spec.ts`](../../apps/cli/src/commands/notifications.spec.ts) drives `dispatchNotifications` with `shared-notifications` fully mocked, covering: each channel firing when its flag and env vars are present, the warning-and-skip path when required env vars are missing (`SLACK_WEBHOOK_URL`, `CLOUDRIFT_WEBHOOK_URL`, the four `CLOUDRIFT_SMTP_*` vars, `GITHUB_TOKEN`/`GITHUB_REPOSITORY`/`GITHUB_EVENT_PATH`), a failed send being logged rather than thrown, the GitHub channel additionally skipping when the triggering event has no `pull_request` (a push/cron run), and multiple channels firing together in one call. `topFindingLines` (shared "top N findings, most-severe-first" helper feeding the webhook/email/GitHub-comment bodies) has its own describe block in the same file.
- **Real-world verification**: Slack was tested live against a real webhook (found and fixed three issues — Slack's `mrkdwn` parser eating a `*`/`_` pair inside a raw IAM policy string, the earlier all-severities design being alert-fatigue-prone, and color/markdown formatting). Email/SMTP and the GitHub PR comment channel are unit-test-verified only; a real-world run of each is still open.
