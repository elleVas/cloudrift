...
Scope is 16 of 29 scanners (see [ADR-0002](../adr/0002-localstack-e2e-scope.md), [ADR-0036](../adr/0036-ec2-underutilized-excluded-from-localstack-e2e.md), and [ADR-0039](../adr/0039-cloudwatch-localstack-incompatibility.md)):
...
- `load-balancer`, `nat-gateway`, and the 3 remaining always-on Phase 5.5 scanners (`vpn-connection-idle`, `transit-gateway-idle-attachment`, `kinesis-provisioned-idle-stream`) are included in the expected-kinds list but treated as soft-missing. For `load-balancer` it's because LocalStack Hobby rejects `elbv2` calls outright. For the other four, the underlying resources are seeded and verified correct (confirmed via direct AWS CLI calls against the container), but `GetMetricStatistics` itself currently fails on LocalStack 4.0 for every CloudWatch-backed scanner — see [ADR-0039](../adr/0039-cloudwatch-localstack-incompatibility.md). That bug also affects the pre-existing hard-required `ebs-idle`, `lambda-underutilized`, `dynamodb-overprovisioned`, and `s3-no-lifecycle`, which as of 2026-06-27 will currently fail the harness outright until either the incompatibility is fixed or those four are moved to soft as well — a known, documented gap, not yet acted on.
