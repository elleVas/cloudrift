...
Alongside waste detection, the CLI has a second, sibling capability built the same hexagonal way but living in its own domain, `cost-analytics` ([ADR-0080](../adr/0080-cost-analytics-extracted-from-cloud-cost.md)): comparing and trending actual AWS spend via Cost Explorer ([ADR-0069](../adr/0069-cost-explorer-integration-billed-api-confirmation.md)). It shares `shared/kernel` but is not an extension of `WastedResource` — a spend comparison has no entity, no waste policy, just aggregate numbers from one external API, so forcing it through the waste model would mean fake entities with no basis in the ubiquitous language. Unlike `dead-resources-domain`, `cost-analytics-domain` doesn't even share `AwsRegion` with `cloud-cost-domain` — Cost Explorer is a single global endpoint, so there is no region value object to share.
...
- **`dead-resources.composition.ts`** mirrors `analyze-waste.composition.ts`'s shape at a fraction of the size: `buildScanners()` is a plain 18-entry array (not split into files the way [ADR-0077](../adr/0077-scanner-registry-split-on-pricing-seam.md) split the 44-entry cost-waste registry — 18 still doesn't warrant it), and `scannerKinds` filtering (from `--scanners` or the wizard's multiselect) works the same way `AnalysisContext.scannerKinds` does.
...
- **The MCP protocol itself lives in `apps/cli`** (`scope:app`), the one project allowed to see every layer — same reason Commander's option-parsing lives there and not in an application package. `mcp.composition.ts` builds the four concrete, AWS-wired use cases (reusing the existing composition roots verbatim); `mcp.command.ts` is pure protocol glue: seven tools (`analyze_cloudrift`, `analyze_cloud_waste`, `analyze_dead_resources`, `analyze_resource_security`, `get_cost_trend`, `get_resource_types`, `get_required_iam_permissions`) registered on an `McpServer` with Zod input schemas. `get_cost_trend` and the three per-domain `analyze_*` tools don't go through `AggregateAnalysisUseCase` — each is a single-domain query, so it wires that one domain's own composition root directly (`CostTrendUseCase`/`defaultCostAnalyticsDeps` for the former, `defaultAnalyzeDeps`/`defaultDeadResourcesDeps`/`defaultResourceSecurityDeps` for the latter three), the same roots the `trend`/`analyze`/`dead-resources`/`resource-security` commands use, minus `trend`'s interactive `confirmCostExplorerCharge` prompt (no terminal to confirm against over MCP). `resolveMcpScope()` in `mcp.composition.ts` factors out the config-loading/region-parsing/account-resolution preamble all four analysis tools share, once a fourth near-identical copy would otherwise have appeared.
...
- **Every registered tool handler is wrapped in a shared `withErrorBoundary`**, catching an *uncaught throw* (as opposed to the *expected* `Result.fail` path above) and converting it to the same `isError` `CallToolResult` shape. Applied uniformly to all seven tools, not only the five AWS-calling ones — the two static tools can't throw today, but the boundary covers them too so a future change to either doesn't need anyone to remember to add it. See [ADR-0086](../adr/0086-mcp-tool-handlers-error-boundary.md).
...
There are three bounded contexts today: **cloud-cost** (waste detection), **cost-analytics** (Cost Explorer spend comparison/trend, split out of `cloud-cost` in [ADR-0080](../adr/0080-cost-analytics-extracted-from-cloud-cost.md) once the two turned out to share no domain model beyond "money"), and **dead-resources** (hygiene findings, [ADR-0078](../adr/0078-dead-resources-parallel-domain.md)) — the `libs/<context>/{domain,application,infrastructure}` structure this repo already used for the first context extended cleanly to the second and third, no changes needed to the pattern itself. They share only `shared/kernel`, with one documented exception: `dead-resources-domain` re-exports `cloud-cost-domain`'s `AwsRegion` rather than duplicating the region-code list (`cost-analytics-domain` doesn't even need this — Cost Explorer has no region concept at all). Nx's `depConstraints` ([ADR-0075](../adr/0075-nx-dep-constraints-layer-enforcement.md)) enforce *layer* isolation (domain/application/infrastructure) but not *context* isolation — nothing stops a future context from importing another context's internals beyond this one deliberate case; it holds by convention and code review, not by a lint rule. Adding a fourth context (e.g. `gcp-cost`, or `resource-security`) follows the same shape.
